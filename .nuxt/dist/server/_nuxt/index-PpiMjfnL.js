...
import { u as useApi } from "./useApi-c2WXwWRZ.js";
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(loadingProspects)) {
        ...
        if (unref(filter) === "pendiente") {
          ...
          _push(`<!--[-->Los prospectos aprobados aparecerán aquí<!--]-->`);
        } else {
          _push(`<!--[-->Los prospectos rechazados aparecerán aquí<!--]-->`);
        }
        _push(`</p>`);
        if (unref(filter) === "pendiente") {
      ...
      if (unref(detectorPhase) === "config") {
        ...
        _push(` ${ssrInterpolate(unref(loadingDetector) ? "Consultando Overpass API (puede tardar 15-60s)..." : "Ejecutar detección")}</button><div><button class="flex w-full items-center justify-between rounded-lg border border-gray-200 bg-white px-5 py-3.5 text-left transition-colors hover:bg-gray-50"><span class="flex items-center gap-2"><svg class="h-5 w-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg><span class="text-sm font-semibold text-ink">Metodología, índices y referencias</span></span><svg class="${ssrRenderClass([unref(showConfigMethodology) ? "rotate-180" : "", "h-5 w-5 text-gray-400 transition-transform duration-200"])}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"></path></svg></button>`);
        if (unref(showConfigMethodology)) {
          _push(`<div class="mt-2 space-y-5"><div class="card p-5"><h3 class="mb-3 font-semibold text-ink">Composición del score (0-100 puntos)</h3><p class="mb-3 text-sm text-slate-custom">Suma ponderada de 6 factores derivados de geometría (Turf.js) y metadatos (OpenStreetMap). Total máximo teórico: 90 pts (no todos los factores se cumplen simultáneamente).</p><div class="overflow-x-auto"><table class="w-full text-sm"><thead><tr class="border-b text-left text-xs uppercase text-gray-500"><th class="py-2 pr-3">Factor</th><th class="py-2 pr-3 text-center">Máx</th><th class="py-2 pr-3">Criterio</th><th class="py-2">Justificación</th></tr></thead><tbody><!--[-->`);
          ...
          _push(`<!--]--></tbody></table></div></div><div class="card p-5"><h3 class="mb-3 font-semibold text-ink">Glosario de índices técnicos</h3><p class="mb-3 text-sm text-slate-custom">Cada columna de la tabla de resultados corresponde a un índice calculado. Hover sobre el encabezado de columna para ver descripción rápida.</p><dl class="divide-y divide-gray-100"><!--[-->`);
          ...
          _push(`<!--]--></dl></div><div class="card p-5"><h3 class="mb-3 font-semibold text-ink">Pipeline de detección</h3><ol class="space-y-2"><li class="flex gap-3 text-sm"><span class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">1</span><span class="text-slate-custom"><strong class="text-ink">Overpass API</strong> — Consulta edificios significativos (nombrados, comerciales, públicos, con niveles) en el bounding box. Solo polígonos (ways) con tag <code>building=*</code>.</span></li><li class="flex gap-3 text-sm"><span class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">2</span><span class="text-slate-custom"><strong class="text-ink">Turf.js</strong> — Calcula área (m²), perímetro, centroide, bounding box, rectangularidad y compacidad Polsby-Popper por edificio.</span></li><li class="flex gap-3 text-sm"><span class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">3</span><span class="text-slate-custom"><strong class="text-ink">Scoring</strong> — Aplica 6 criterios ponderados. Estima NDVI potencial, carga estructural, zona sísmica, reducción CO₂ y captación pluvial.</span></li><li class="flex gap-3 text-sm"><span class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-primary text-xs font-bold text-white">4</span><span class="text-slate-custom"><strong class="text-ink">Ranking</strong> — Ordena por score, retorna max 200 candidatos. Admin filtra, selecciona y envia a cola de prospectos.</span></li></ol></div><div class="card p-5"><div class="grid gap-5 md:grid-cols-2"><div><h3 class="mb-2 font-semibold text-ink">Fuentes de datos</h3><ul class="space-y-1 text-sm text-slate-custom"><li><strong>OpenStreetMap</strong> — Huellas de edificios, tipo, niveles, material techo. Licencia ODbL. Datos colaborativos, cobertura variable.</li><li><strong>Turf.js v7</strong> — Librería JS de análisis geoespacial. MIT License.</li><li><strong>NTC-CDMX 2017</strong> — Normas Técnicas Complementarias, Cap. 6. Carga mín TVLE: 100 kg/m².</li><li><strong>Cervantes Nájera (2021)</strong> — Tesis maestría CIIEMAD-IPN. CO₂: 0.968 kg/m²/año.</li><li><strong>Cervantes Nájera (2025)</strong> — Tesis doctoral CIIEMAD-IPN. Captación: 19.5 L/m²/año.</li><li><strong>Atlas de Riesgos CDMX</strong> — SGIRPC. Zonificación sísmica simplificada.</li></ul></div><div><h3 class="mb-2 font-semibold text-ink">Limitaciones</h3><ul class="space-y-1 text-sm text-slate-custom"><li>No utiliza imágenes satelitales. NDVI potencial es un <strong>proxy morfológico</strong>, no índice espectral real.</li><li>Datos OSM pueden ser incompletos (edificios sin nombre, sin niveles, sin material de techo).</li><li>Zona sísmica es aproximación por latitud, no consulta al mapa de microzonificación real.</li><li>Carga estructural requiere <strong>dictamen de ingeniero estructurista</strong> para validación.</li><li>Bbox limitado a ~5×5 km por restricciones de Overpass API pública.</li><li>Costo: <strong>$0</strong> — OSM y Overpass son gratuitos. Para NDVI real se requiere Google Earth Engine (también gratuito con cuenta).</li></ul></div></div></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div>`);
      ...
      if (unref(detectorPhase) === "results") {
        ...
        ssrRenderList(unref(paginated), (r) => {
          _push(`<!--[--><tr class="${ssrRenderClass([unref(selected).has(unref(allResults).indexOf(r)) ? "bg-primary-50/40" : "", "cursor-pointer transition-colors hover:bg-gray-50"])}"><td class="px-3 py-2"><input type="checkbox"${ssrIncludeBooleanAttr(unref(selected).has(unref(allResults).indexOf(r))) ? " checked" : ""}></td><td class="px-1 py-2"><button class="rounded p-0.5 text-gray-400 hover:bg-gray-200 hover:text-ink" title="Ver desglose del score y datos técnicos"><svg class="${ssrRenderClass([unref(expandedRow) === unref(allResults).indexOf(r) ? "rotate-90" : "", "h-4 w-4 transition-transform"])}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"></path></svg></button></td><td class="whitespace-nowrap px-3 py-2 font-mono text-xs text-slate-custom">${ssrInterpolate(r.indices?.refId)}</td><td class="max-w-[180px] truncate px-3 py-2 font-medium text-ink"${ssrRenderAttr("title", r.nombre)}>${ssrInterpolate(r.nombre)}</td><td class="whitespace-nowrap px-3 py-2 text-xs">${ssrInterpolate(r.tipo)}</td><td class="px-3 py-2 text-right"><span class="${ssrRenderClass(["rounded-full px-2 py-0.5 text-xs font-bold", scoreClass(r.score)])}">${ssrInterpolate(r.score)}</span></td><td class="whitespace-nowrap px-3 py-2 text-right tabular-nums">${ssrInterpolate(r.areaM2?.toLocaleString("es-MX"))}</td><td class="px-3 py-2 text-right tabular-nums">${ssrInterpolate(r.indices?.ndviPotencial?.toFixed(2))}</td><td class="px-3 py-2 text-right tabular-nums">${ssrInterpolate((r.indices?.rectangularidad * 100)?.toFixed(0))}%</td><td class="px-3 py-2 text-right tabular-nums">${ssrInterpolate((r.indices?.compacidad * 100)?.toFixed(0))}%</td><td class="px-3 py-2"><span class="${ssrRenderClass(["whitespace-nowrap rounded px-1.5 py-0.5 text-xs", cargaClass(r.indices?.cargaEstimada)])}">${ssrInterpolate(r.indices?.cargaEstimada?.replace(/_/g, " "))}</span></td><td class="px-3 py-2 text-xs">${ssrInterpolate(r.indices?.zonaSismica)}</td><td class="px-3 py-2 text-right tabular-nums">${ssrInterpolate(r.indices?.reduccionCO2)}</td><td class="whitespace-nowrap px-3 py-2 text-right tabular-nums">${ssrInterpolate(r.indices?.captacionPluvial?.toLocaleString("es-MX"))}</td><td class="px-3 py-2 text-right tabular-nums">${ssrInterpolate(r.indices?.niveles || "—")}</td></tr>`);
          if (unref(expandedRow) === unref(allResults).indexOf(r)) {
            ...
            _push(`<!--]--></div></div><div><h4 class="mb-2 text-xs font-bold uppercase tracking-wider text-gray-500">Datos técnicos completos</h4><dl class="grid grid-cols-1 gap-x-4 gap-y-1.5 text-xs sm:grid-cols-2"><dt class="text-slate-custom">Ref OSM</dt><dd class="font-mono font-medium">${ssrInterpolate(r.indices?.refId)}</dd><dt class="text-slate-custom">Perímetro</dt><dd class="font-medium tabular-nums">${ssrInterpolate(r.indices?.perimetroM?.toLocaleString("es-MX"))} m</dd><dt class="text-slate-custom">Orientación</dt><dd class="font-medium tabular-nums">${ssrInterpolate(r.indices?.orientacion?.toFixed(0))}°</dd><dt class="text-slate-custom">Isla de calor</dt><dd class="font-medium tabular-nums">${ssrInterpolate((r.indices?.islaTerApprox * 100)?.toFixed(2))}% del ref. 79,584 m²</dd><dt class="text-slate-custom">Material techo</dt><dd class="font-medium">${ssrInterpolate(r.indices?.materialTecho)}</dd><dt class="text-slate-custom">Coordenadas</dt><dd class="font-medium tabular-nums">${ssrInterpolate(r.lat?.toFixed(6))}, ${ssrInterpolate(r.lng?.toFixed(6))}</dd></dl>`);
            if (r.reasons?.length) {
        ...
        if (unref(showMethodology)) {
          ...
          _push(`<!--]--></dl></div><div><h4 class="mb-1 font-semibold text-ink">Fuentes</h4><ul class="list-inside list-disc text-xs text-slate-custom"><li>OpenStreetMap (Overpass API) — ODbL</li><li>Turf.js v7 — MIT</li><li>NTC-CDMX 2017 — carga mín TVLE: 100 kg/m²</li><li>Cervantes Nájera (2021, 2025) — CIIEMAD-IPN</li><li>Atlas de Riesgos CDMX — SGIRPC</li></ul></div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div></div>`);
...
//# sourceMappingURL=index-PpiMjfnL.js.map
