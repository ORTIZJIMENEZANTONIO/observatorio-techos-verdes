  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (__props.totalPages > 1) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "flex flex-col items-center gap-3 sm:flex-row sm:justify-between" }, _attrs))}><p class="text-xs text-slate-custom"> Mostrando <span class="font-semibold text-ink tabular-nums">${ssrInterpolate(unref(startItem))}-${ssrInterpolate(unref(endItem))}</span> de <span class="font-semibold text-ink tabular-nums">${ssrInterpolate(__props.totalItems)}</span></p><nav class="flex items-center gap-1" aria-label="Paginaci\xF3n"><button class="${ssrRenderClass([__props.currentPage === 1 ? "text-gray-300 cursor-not-allowed" : "text-ink-muted hover:bg-gray-100", "flex h-8 w-8 items-center justify-center rounded-lg text-sm transition-colors"])}"${ssrIncludeBooleanAttr(__props.currentPage === 1) ? " disabled" : ""} aria-label="P\xE1gina anterior"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></button><!--[-->`);
...
//# sourceMappingURL=PaginationControls-BAcytpD9.mjs.map
